...
} as Record<string, { coords: Vector3; point: Vector3 }>;
...
function getRgbColors(): { hair: number[][]; makeUp: number[][] } {
	...
	for (let i = 0; i < GetNumHairColors(); i++) {
		hair.push(GetPedHairRgbColor(i));
	}

	for (let i = 0; i < GetNumMakeupColors(); i++) {
		makeUp.push(GetMakeupRgbColor(i));
	}
...
export async function wearClothes(data: PedAppearance, typeClothes: string): Promise<void> {
	const { animations, props } = DATA_CLOTHES[typeClothes as keyof typeof DATA_CLOTHES];
	...
	const loopTrough = isMale ? male : female;

	for (const [componentId] of loopTrough) {
		for (const { component_id, drawable, texture } of components) {
...
export async function removeClothes(typeClothes: string): Promise<void> {
	...
	for(const [componentId, drawableId] of loopTrough) {
