...
import { FACE_FEATURES, HEAD_OVERLAYS, HAIR_DECORATIONS, PED_COMPONENTS_IDS, PED_PROPS_IDS, EYE_COLORS, } from "~/constants";
...
export const pedModels: string[] = JSON.parse(LoadResourceFile(GetCurrentResourceName(), "peds.json"));
...
function getPedComponents(ped: number): PedComponent[] {
	return PED_COMPONENTS_IDS.map(component => {
		return {
...
function getPedProps(ped: number): PedProp[] {
	return PED_PROPS_IDS.map(component => {
		return {
			prop_id: component,
			drawable: GetPedPropIndex(ped, component),
			texture: GetPedPropTextureIndex(ped, component),
		};
	});
}
...
function getPedFaceFeatures(ped: number): PedFaceFeatures {
	return FACE_FEATURES.reduce((object, feature, index) => {
...
function getPedHeadOverlays(ped: number): PedHeadOverlays {
	return HEAD_OVERLAYS.reduce((object, overlay, index) => {
...
function getPedHairDecoration(ped: number, hairStyle: number): HairDecoration {
	...
	return HAIR_DECORATIONS[hairDecorationType].find(
